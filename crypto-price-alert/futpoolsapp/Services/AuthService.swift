...
final class AuthService: ObservableObject {
    ...
    func register(
        ...
        do {
            ...
            // Flush any APNs token captured during onboarding (the user may
            // have granted push permission before this account existed).
            await PushService.shared.registerPendingToken()
        } catch {
            handleError(error)
        }
    }
    ...
    /// Permanently delete the signed-in account (App Store Guideline
    /// 5.1.1(v)). On success the backend wipes the user + all their records;
    /// we drop the local session so RootView returns to the auth flow.
    /// Returns false on failure (error surfaced via `errorMessage`).
    @discardableResult
    func deleteAccount() async -> Bool {
        guard let t = token else { return false }
        clearError()
        do {
            try await client.requestVoid(method: "DELETE", path: "/users/me", token: t)
            PushService.shared.clearLocalToken()
            logout()
            return true
        } catch {
            handleError(error)
