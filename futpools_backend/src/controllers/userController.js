...
exports.getMe = async (req, res) => {
  try {
    ...
    const payload = {
      ...
      isAdmin,
      // Payout/banking info so the edit-profile form can prefill. Normalized
      // through serializePayout so legacy accounts (payout undefined) return a
      // stable empty shape instead of null.
      payout: serializePayout(req.user.payout),
    };
 ...
 * where error is the {message, code, field} shape updateMe sends back.
 *
...
exports.updateMe = async (req, res) => {
  try {
    const { displayName, username, email, currentPassword, payout } = req.body || {};
    ...
    // Payout/banking info. Only touched when the client sends a `payout`
    // object — a name-only edit leaves any saved banking details intact.
    if (payout !== undefined && payout !== null) {
      const result = validatePayout(payout);
      if (!result.ok) {
        return res.status(400).json(result.error);
      }
      user.payout = result.payout;
      user.markModified('payout');
    }
